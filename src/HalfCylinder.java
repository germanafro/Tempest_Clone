 ...
 *
 */
public class HalfCylinder extends Primitive {
	
	private int r;
	public HalfCylinder(int x, int y, int z, int r, int scale, Game game) {
	...
	public HalfCylinder(int x, int y, int z, int r, int scale, Game game, String texture) {
...
// ================================== 1. Define vertices ==================================
		...
		double stepr = 180d/y; // num of sides
		float[] vertices;
		float[] normals;
		...
		for (int i = 0; i < this.getxTiles(); i ++){
			for (int j = 0; j < this.getyTiles() + 1; j ++){
				double rad =  Math.toRadians(180f + j*stepr);
